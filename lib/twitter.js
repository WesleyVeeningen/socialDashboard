...
export async function getTwitterData() {
  ...
  try {
    ...
    return {
      ...
      recentTweets: (tweetsJson.data ?? []).map((t) => ({
        ...t,
...
export async function postTwitterReply(tweetId, text, replyTo) {
  ...
  try {
    // If replying to a specific comment/reply tweet, use that ID; otherwise reply to the original tweet
    const replyToId = replyTo ?? tweetId;
